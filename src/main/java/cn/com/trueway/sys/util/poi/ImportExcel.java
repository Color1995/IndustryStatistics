package cn.com.trueway.sys.util.poi;
...
public class ImportExcel {
    ...
    //解析excel文件方法一
    public static List<String[]> analysisFileAppoint(MultipartFile file, int sheetNum) throws IOException {
        //获取workbook对象
        ...
        if(workbook != null & sheet != null){
        	System.out.println("Sheet总页数: " + workbook.getNumberOfSheets());
            	
        	//创建对象，把每一行作为一个String数组，所以数组存到集合中
            List<String[]> resultList = new ArrayList<>();
            
			/*
			 * if(sheet == null){ hashMap.put("文件sheet为空!",arrayList); return hashMap; }
			 */
            ...
            for(int rowNum = 0;rowNum <= lastRowNum;rowNum++){
                ...
                for(int cellNum = firstCellNum;cellNum < lastCellNum;cellNum++){
					...
					/*
					 * if( cell == null || "".equals(cell) || cell.getCellType()==
					 * Cell.CELL_TYPE_BLANK ){ cell = null; }
					 */
    ...
    //解析excel文件方法二
    public static List<ArrayList<String>> analysisSheetTwo(MultipartFile file, int sheetNum) throws IOException {
        //获取workbook对象
        Workbook workbook = null;
        String filename = file.getOriginalFilename();
        InputStream inputStream = file.getInputStream();
        ...
        if(workbook != null & sheet != null){
         ...
         //   String[] strings = null;
            for(int rowNum = 0;rowNum <= lastRowNum;rowNum++){
                ...
                if(false == pattern.matcher(str).matches() || "".equals(str)) {
					continue;
				}
                // 获取当前行的开始列和结束列
                short firstCellNum = row.getFirstCellNum();
           ...
           //     strings = new String[55];
                ...
                String cellValue = null;
                //遍历每一行,获取每一列值
                for(int cellNum = firstCellNum;cellNum < 52;cellNum++){
                    Cell cell = row.getCell(cellNum);
					/*
					 * if( cell == null || "".equals(cell) || cell.getCellType()==
					 * Cell.CELL_TYPE_BLANK ){ cell = null; }
					 ...
					 */
                    result.add(cellNum, cellValue);
                 //   strings[cellNum] = cellValue;
                }
                resultList.add((ArrayList<String>) result);
            }
    ...
    //解析excel文件
    public static LinkedHashMap<String, ArrayList<String[]>> analysisFile(MultipartFile file, int rows) throws IOException {
        ...
        if(workbook != null){
            ...
            for(int sheetNum = 0;sheetNum < workbook.getNumberOfSheets();sheetNum++){
                ...
                for(int rowNum = firstRowNum;rowNum <= lastRowNum;rowNum++){
                    ...
                    int totalCellNums = row.getPhysicalNumberOfCells();
                    //开辟结束列大小的数组
                    strings = new String[lastCellNum];
                    //循环当前行
                    for(int cellNum = firstCellNum;cellNum < lastCellNum;cellNum++){
                        Cell cell = row.getCell(cellNum);
						/*
                ...
                // 每个sheet表解析完成后 存入hashmap
                hashMap.put(Integer.toString(sheetNum),arrayList);
            }
        }
        inputStream.close();
        return (LinkedHashMap<String, ArrayList<String[]>>) hashMap;
    }
    ...
    public String saveFile(MultipartFile file) throws IOException {
        ...
        //传输文件到指定路径中
        String path = "D:/uploading/"+originalFilename;
